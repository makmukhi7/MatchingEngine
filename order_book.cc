...
void RemoveFromOrderMap(MapType& m, MapIteratorType map_itr, OrderList::iterator order_list_itr, PriceIndex& price_index) {
    if (map_itr->second.size() == 1) {
        // If there's only one order for that price, we can remove the map entry itself. And also remove from price index.
...
std::pair<MapTypeIterator, OrderList::iterator> AddToOrderMap(MapType& m, Order o) {
    auto [order_map_itr, success] = m.emplace(std::make_pair(o.price, OrderList()));
    OrderList& order_list = order_map_itr->second;
    OrderList::iterator order_list_itr = order_list.insert(order_list.end(), std::move(o));
    return { order_map_itr, order_list_itr };
}

// Following helper functions help keep matching logic agnostic of if the incoming order is buy or sell.
...
void OrderBook::ExecuteTrades(Order& incoming_order, OrderList& order_list) {
    while (incoming_order.qty > 0 && !order_list.empty()) {
        ...
        if (te.qty == incoming_order.qty) {
            ...
            OrderPartiallyFilled o{ .order_id = incoming_order.id, .remaining = incoming_order.qty };
            os_ << o << std::endl;
        }
        if (te.qty == resting_order.qty) {
            ...
            OrderPartiallyFilled o{ .order_id = resting_order.id, .remaining = resting_order.qty };
            os_ << o << std::endl;
        }
    }
}
...
void OrderBook::MatchOrders(Order& incoming_order, MapType& resting_orders, MatchingFunction match) {
    for (auto itr = resting_orders.begin(); itr != resting_orders.end() && incoming_order.qty > 0;) {
...
void OrderBook::AddOrder(Order o) {
    ...
    if (price_index_itr != price_index_.end()) {
        ...
        OrderEntry entry;
        if (o.side == Side::kSell) {
            auto [map_itr, list_itr] = AddToOrderMap<SellOrderMap, SellOrderMap::iterator>(sell_orders_, std::move(o));
            entry.first.sell_order_map_it = map_itr;
            entry.second = list_itr;
        }
        else {
            auto [map_itr, list_itr] = AddToOrderMap<BuyOrderMap, BuyOrderMap::iterator>(buy_orders_, std::move(o));
...
void OrderBook::ProcessOrder(const AddOrderRequest& req) {
    ...
    Order incoming_order{ .id = req.order_id, .side = req.side, .qty = req.qty, .price = req.price };
...
void OrderBook::ProcessOrder(const CancelOrderRequest& req) {
    ...
    if (order_list_itr->side == Side::kBuy) {
        RemoveFromOrderMap(buy_orders_, order_map_itr.buy_order_map_it, order_list_itr, price_index_);
    }
    else {
        RemoveFromOrderMap(sell_orders_, order_map_itr.sell_order_map_it, order_list_itr, price_index_);
    }
}
